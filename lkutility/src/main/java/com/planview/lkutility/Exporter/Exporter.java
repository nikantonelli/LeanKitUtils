...
public class Exporter {
    ...
    private static void createItemRowFromCard(XSSFSheet chgSht, Integer chgRowIdx, XSSFSheet itemSht, Integer itmRowIdx, Card c, Field[] pbFields)
            {
                ...
                try {
                    switch (pbFields[i].getName()) {
                        ...
                        case "lane": {
                            Object fv = c.getClass().getField(pbFields[i].getName()).get(c);
                            if (fv != null) {
                                itmRow.createCell(i + 1, CellType.STRING).setCellValue(Utils.getLanePathFromId(cfg, cfg.source, ((Lane)fv).id));
                            }
                            break;
                        }
                        case "srcID": {
                            ...
                            if (cfg.addComment) {
                                chrRowIncr++;
                                createChangeRow(chgSht, chgRowIdx + chrRowIncr, itmRowIdx, "Modify", "Comment", Utils.getUrl(cfg, cfg.source) + "/card/" + c.id);
                            }
                            break;
                        }
                        case "tags": {
                        ...
                        case "taskBoardsStats": {
                            ...
                            if (c.taskBoardStats.totalCount != 0){
                                ArrayList<Card> tasks = Utils.readTasksFromCard(cfg, cfg.source, c.id);
        ...
        public static void createChangeRow(XSSFSheet chgSht, Integer chgRowIdx, Integer itmRowIdx, String action, String field, String value){
            ...
            chgRow.createCell(localCellIdx++, CellType.STRING)
